...
async function writeVersionFile(version) {
  const content = `// Auto-generated by scripts/fetch-qs-jwt-version.mjs – do not edit\nexport const version = '${version}';\n`;
...
async function main() {
  try {
    const headers = { "User-Agent": "qs-jwt-docs-build-script" };
    ...
    console.log(`[qs-jwt-docs] Latest ${REPO} version: ${tag}`);
  } catch (err) {
    console.warn(
      `[qs-jwt-docs] Warning: Could not fetch latest release tag: ${err.message}`
    );
    // If the file already exists, keep it. Otherwise write a safe default.
    try {
      const existing = await fs.readFile(outFile, "utf8");
      if (existing) {
        console.log("[qs-jwt-docs] Using existing generated version.js");
        return;
      }
    } catch {}
    const fallback = "v0.0.0";
    await writeVersionFile(fallback);
    console.log(`[qs-jwt-docs] Wrote fallback version ${fallback}`);
  }
}
